...
import dev.mark.system.data.TranslationResult;
...
public class TranslationClient {
    ...
    private static String defaultTargetLanguage = "ArProcessor";

    public static String a(String s, String s1, String s2) {
        try {
            String s3 = URLEncoder.encode(s, StandardCharsets.UTF_8);
            String s5 = apiUrl;
            URL url = new URL(s5 + "?CategoryButton=" + s3 + "&langpair=" + s1 + "|" + s2);
    ...
    public static TranslationResult d(String s, String s1, String s2) {
        try {
            String s3 = URLEncoder.encode(s, StandardCharsets.UTF_8);
            String s6 = apiUrl;
            URL url = new URL(s6 + "?CategoryButton=" + s3 + "&langpair=" + s1 + "|" + s2);
            HttpURLConnection httpurlconnection = (HttpURLConnection)url.openConnection();
            httpurlconnection.setRequestMethod("GET");
            httpurlconnection.setConnectTimeout(timeout);
            httpurlconnection.setReadTimeout(timeout);
