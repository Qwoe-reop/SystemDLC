...
import dev.mark.system.util.ColorUtil;
...
public final class BuiltText implements Renderable {
    private static final ShaderProgramKey aoq = ShaderUtil.a("msdf_font", VertexFormats.field_1575, Defines.field_53930);
    ...
    private static void safeSetUniformInt(ShaderProgram shader, String name, int value) {
        GlUniform uniform = shader.method_34582(name);
        if (uniform != null) {
    ...
    public BuiltText(MSDFFont msdffont, String s, float f, float f1, int i, float f2, float f3, int j, float f4) {
        this(msdffont, s, f, f1, i, f2, f3, j, f4, true);
    }

    public BuiltText(MSDFFont msdffont, String s, float f, float f1, int i, float f2, float f3, int j, float f4, boolean flag) {
        this(msdffont, s, f, f1, i, f2, f3, j, f4, flag, -14737633, -1, 0.5F, 1.0F, 0.0F, 0.0F, 0.0F, 0.1F, 0.15F, 0.25F);
    }
    ...
    private void b(Matrix4f matrix4f, float f, float f1, float f2, float f3) {
        if (this.kH != null && !this.kH.isEmpty()) {
            ...
            if (this.dg > 0.0F && (this.PT != 0.0F || this.Pm != 0.0F || this.ek > 0.0F)) {
                safeSetUniform(shaderprogram, "Range", this.SH.d().a());
                safeSetUniform(shaderprogram, "Thickness", this.akR);
                safeSetUniform(shaderprogram, "Smoothness", this.UJ + this.ek * 2.0F);
                safeSetUniformInt(shaderprogram, "Outline", 0);
                safeSetUniformInt(shaderprogram, "UseGradient", 0);
                float f4 = this.dg * f3;
                int ix = this.ek > 0.0F ? 9 : 1;
                float f5 = f4 / ix;
                safeSetUniform(shaderprogram, "TextColor", 0.0F, 0.0F, 0.0F, f5);
                BufferBuilder bufferbuilder = Tessellator.method_1348().method_60827(DrawMode.field_27382, VertexFormats.field_1575);
            ...
            if (this.WC > 0.0F && this.amR > 0.0F) {
                safeSetUniform(shaderprogram, "Range", this.SH.d().a());
                safeSetUniform(shaderprogram, "Thickness", this.akR - this.amR * 0.3F);
                safeSetUniform(shaderprogram, "Smoothness", this.UJ + this.amR * 3.0F);
                safeSetUniformInt(shaderprogram, "Outline", 0);
                safeSetUniformInt(shaderprogram, "UseGradient", 0);
                float f8 = this.WC * f3;
                byte b1 = 16;
                float f9 = f8 / b1;
                safeSetUniform(shaderprogram, "TextColor", 1.0F, 1.0F, 1.0F, f9);
                BufferBuilder bufferbuilder1 = Tessellator.method_1348().method_60827(DrawMode.field_27382, VertexFormats.field_1575);
                ...
                BufferRenderer.method_43433(bufferbuilder1.method_60800());
            }

            safeSetUniform(shaderprogram, "Range", this.SH.d().a());
            safeSetUniform(shaderprogram, "Thickness", this.akR);
            safeSetUniform(shaderprogram, "Smoothness", this.UJ);
            safeSetUniformInt(shaderprogram, "Outline", flag ? 1 : 0);
            if (flag) {
                safeSetUniform(shaderprogram, "OutlineThickness", this.QN);
                ColorUtil.e(this.DY, i);
                safeSetUniform(shaderprogram, "OutlineColor", i[0], i[1], i[2], i[3] * f3);
            }

            ColorUtil.e(this.FI, GT);
            safeSetUniform(shaderprogram, "TextColor", GT[0], GT[1], GT[2], GT[3] * f3);
            String s = this.SH.f();
            boolean flag1 = "native_bootstrap_alt".equals(s);
            boolean flag2 = (this.FI & 16777215) == 16777215;
            boolean flag3 = this.ee && !flag1 && flag2;
            safeSetUniformInt(shaderprogram, "UseGradient", flag3 ? 1 : 0);
            if (flag3) {
                ColorUtil.e(this.jd, aaL);
                ColorUtil.e(this.fG, Ld);
                safeSetUniform(shaderprogram, "GradientColor1", aaL[0], aaL[1], aaL[2], aaL[3]);
                safeSetUniform(shaderprogram, "GradientColor2", Ld[0], Ld[1], Ld[2], Ld[3]);
                safeSetUniform(shaderprogram, "GradientCenter", this.qp, this.ZJ);
            }

            BufferBuilder bufferbuilder2 = Tessellator.method_1348().method_60827(DrawMode.field_27382, VertexFormats.field_1575);
            this.SH
                .b(matrix4f, bufferbuilder2, this.kH, this.lN, (this.akR + this.QN * 0.5F) * 0.5F * this.lN, this.Nu, f, f1 + this.SH.e().d() * this.lN, f2, b0);
            BufferRenderer.method_43433(bufferbuilder2.method_60800());
