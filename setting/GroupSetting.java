...
public class GroupSetting extends Setting {
    ...
    public void addSetting(Setting setting) {
        if (setting != null) {
            ...
            try {
                if (this.settings.stream().anyMatch(setting2 -> setting2.getName().equals(setting.getName()))) {
                    return;
                }
        ...
        if (asetting != null && asetting.length != 0) {
            ...
            try {
                for (Setting setting : asetting) {
                    if (setting != null && !this.settings.stream().anyMatch(setting2 -> setting2.getName().equals(setting.getName()))) {
                        this.settings.add(setting);
                    }
                }
            } finally {
    ...
    public void removeSetting(String s) {
        if (s != null && !s.trim().isEmpty()) {
            ...
            try {
                this.settings.removeIf(setting -> setting.getName().equals(s));
            } finally {
                this.lock.writeLock().unlock();
            }
        }
    ...
    public Setting getSetting(String s) {
        if (s != null && !s.trim().isEmpty()) {
            ...
            Setting setting;
            try {
                setting = this.settings.stream().filter(setting1 -> setting1.getName().equals(s)).findFirst().orElse(null);
            } finally {
